...
static esp_err_t get_handler(httpd_req_t *req)
{
    ...
    else if (strcmp(filename, "/readADC") == 0)               // Enviar valores de graficos
    {
        ...
        httpd_resp_sendstr_chunk(req, buffer); //humedad
        
        httpd_resp_sendstr_chunk(req, "</analog>");
        httpd_resp_sendstr_chunk(req, "<analog>");
        
        n=sprintf (buffer, "%f", (pot*1.0)); 
        httpd_resp_sendstr_chunk(req, buffer); //Luminosidad
        httpd_resp_sendstr_chunk(req, "</analog>");
        httpd_resp_sendstr_chunk(req, "</inputs>");
    ...
    //httpd_resp_sendstr_chunk(req, "");
        ...
        //temperatura+=1; if(temperatura > 70) temperatura=0;
        //humedad-=1; if(humedad < 10) humedad=90;
    }
